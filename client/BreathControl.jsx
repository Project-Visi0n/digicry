...
export default function BreathControl({
  ...
  useEffect(() => {
    function animate(now) {
      ...
      setT(progress);
      if (elapsed < duration) {
        if (!paused && typeof globalCtx.requestAnimationFrame === "function") {
  ...
  // Color interpolation: mint for triangle, pink for circle
  // --mint: #B8F2E6, --pink: #FFA69E
  function lerpColor(a, b, tt) {
    // a, b: hex strings, tt: 0-1
    const ah = a.replace("#", "");
    const bh = b.replace("#", "");
  ...
  // Move back to the right (previously -140px)
  return (
    <svg
      width="800"
      height="800"
